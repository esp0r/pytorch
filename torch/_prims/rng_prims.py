...
def register_run_with_start_end_rng_state_op():
    ...
    class RunWithRngStartEndState(HigherOrderOperator):
        def __init__(self):
            super().__init__("run_with_start_end_rng_state", cacheable=True)
    ...
    run_with_start_end_rng_state = RunWithRngStartEndState()
    ...
    def impl_backend_select(start_rng_state, end_rng_state, op, *args, **kwargs):
        ...
        if device != "cuda":
            raise RuntimeError(
                f"run_with_start_end_rng_state only supports CUDA device, got {device}. "
